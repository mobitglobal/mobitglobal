...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Mobit Global database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Mobit Global datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Mobit Global datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Mobit Global 0.7 datadir."
    ;;
  3)
    echo "Detected Mobit Global pre-0.8 datadir."
    ;;
  4)
    echo "Detected Mobit Global 0.8 datadir."
    ;;
esac
