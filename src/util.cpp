...
// Copyright (c) 2017 The Mobit Global Core developers
...
const char * const BITCOIN_CONF_FILENAME = "mobitglobal.conf";
const char * const BITCOIN_PID_FILENAME = "mobitglobald.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "mobitglobal" is a composite category enabling all mobitglobal-related debug output
            if(ptrCategory->count(string("mobitglobal"))) {
...
#else
    const char* pszModule = "mobitglobal";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Mobit Global Core
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MobitGlobalCore
    // Mac: ~/Library/Application Support/MobitGlobalCore
    // Unix: ~/.mobitglobalcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mobit Global Core";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Mobit Global Core";
#else
    // Unix
    return pathRet / ".mobitglobalcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty mobitglobal.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override mobitglobal.conf
