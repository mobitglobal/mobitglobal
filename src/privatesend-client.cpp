...
void CPrivateSendClient::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all Mobit Global related functionality
...
#ifdef _MSC_VER
	CRecipient rp = { scriptCollateral, CPrivateSend::GetMaxCollateralAmount(), false };
	vecSend.push_back(rp);
#else
...
#ifdef _MSC_VER
		CRecipient rp = {
			scriptCollateral, CPrivateSend::GetMaxCollateralAmount(), false
		};
		vecSend.push_back(rp);
#else
        vecSend.push_back((CRecipient){ scriptCollateral, CPrivateSend::GetMaxCollateralAmount(), false });
...
{
    if(fLiteMode) return; // disable all Mobit Global specific functionality
    ...
    RenameThread("mobitglobal-ps-client");
